...
- **扫描与索引**：增量扫描（按大小+修改时间跳过未变文件）、并行哈希（多线程管线）、进度百分比/速度/预计剩余时间/当前处理路径、可取消、失效目录自动跳过、逐文件失败清单（最近 20 条）、外部删除的文件在下次扫描标记缺席。自定义排除规则与最小文件大小（设置页 / CLI）。>16 MB 文件两级哈希：先比前 64 KB 指纹，同桶才计算全量。启动时可用已保存目录自动增量扫描（设置页可关）。
- **检测能力**：精确重复（BLAKE3 + 大小，可自动处理）；相似照片（dHash ≤4/64，仅人工查看）；相似文档（TXT/MD/CSV/JSON/XML/HTML 的 SimHash ≤8/64，仅人工查看）；音频/视频指纹未接入，界面如实标注。
- **审核与处理**：重复组支持路径筛选/最小大小/排序与总数分页，操作后保留已加载页；智能标记（保留最新/最旧/最短路径）；点击缩略图应用内放大预览（EXIF 方向、磁盘缓存）、系统程序打开、文件管理器定位；"标记删除 → 选择回收站或直接永久删除"两步语义；超 1 GB 文件提示回收站会占用双倍空间；保护路径规则禁止误删；CSV 报告导出（界面 / CLI）。
...
数据库结构带自动迁移：老版本库在打开时按迁移框架逐步升级（当前 schema v2，新增两级哈希所需的 `quick_hash` 列），无需人工干预即可沿用已有索引。
